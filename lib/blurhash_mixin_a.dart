...
mixin BlurHashMixinA {
  Future<Tuple2<Image, bool>> getBlurImage({
    ...
    try {
        ...
        if (isRemoteImage) {
          ...
          imageBytes = fileExists(cover)
              ...
              : await getBytesFromRemoteImage(
                  uri: Uri.parse(AppAssets.defaultCoverUrl),
                );
        }
      ...
      final imageBytes = getBytesFromLocalImage(
        path: AppAssets.icoCDPlaceholder,
      );
      final blurHash = blurHashImage(imageBytes: imageBytes);
  ...
  Future<Tuple2<Image, bool>> getMediaBlurCover({
    ...
    return getBlurImage(
      cover: cover ?? AppAssets.defaultCoverUrl,
      hash: hash,
